...
import { fetchRevenue, deleteRevenue } from '../../features/revenue/revenueSlice';
import { Header } from '../../components/layout/Header';
import { Card } from '../../components/ui/Card';
import { Badge } from '../../components/ui/Badge';
import { ConfirmDialog } from '../../components/ui/ConfirmDialog';
import { Plus, Wallet, Search, Edit, Trash2 } from 'lucide-react';
...
export const RevenueList: React.FC = () => {
  ...
  const { items: revenue, status } = useSelector((state: RootState) => state.revenue);

  const [search, setSearch] = useState('');
  const [deleteTarget, setDeleteTarget] = useState<RevenueLog | null>(null);
  const [isDeleting, setIsDeleting] = useState(false);

  useEffect(() => {
    if (status === 'idle') dispatch(fetchRevenue());
  }, [dispatch, status]);

  const filtered = revenue.filter(log =>
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await dispatch(deleteRevenue(deleteTarget.id)).unwrap();
    } finally {
      setIsDeleting(false);
  ...
  return (
    <>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search by vehicle, route or client..."
            />
          </div>
          ...
          {status === 'loading' ? (
            ...
            <div className="text-center p-12 bg-[var(--bg-surface)] rounded-xl border border-[var(--border)] mt-8">
              <Wallet size={48} className="mx-auto text-[var(--text-muted)] mb-4" />
              <h3 className="text-lg font-medium mb-2">No income logged</h3>
              <p className="text-sm text-[var(--text-muted)] mb-4">Record your first daily collection or trip payment.</p>
              <button
                ...
                <Card key={log.id} className="p-4">
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex flex-col">
                      <span className="font-bold text-lg text-[var(--primary)]">{formatCurrency(log.amount)}</span>
                      <span className="text-xs text-[var(--text-muted)]">{formatRelativeTime(log.date)}</span>
                    </div>
                    <Badge variant={log.payment_status === 'paid' ? 'success' : 'warning' as any}>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    {log.revenue_type === 'daily_collection' && log.route && (
                      <div className="text-xs text-[var(--text-muted)] flex-1 truncate">Route: {log.route}</div>
                    )}
                    {log.revenue_type === 'trip' && log.client && (
                      <div className="text-xs text-[var(--text-muted)] flex-1 truncate">Client: {log.client}</div>
                    )}
                  </div>
                  ...
                  <div className="flex gap-2 mt-3 pt-3 border-t border-[var(--border)]">
                    <button
                      onClick={() => navigate(`/revenue/edit/${log.id}`)}
      ...
      <ConfirmDialog
        ...
        message={`Delete this ${deleteTarget?.revenue_type === 'daily_collection' ? 'daily collection' : 'trip payment'} of ${deleteTarget ? formatCurrency(deleteTarget.amount) : ''}? This cannot be undone.`}
